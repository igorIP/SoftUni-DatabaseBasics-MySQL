...
WHERE 
	-- date_format(u.birthdate, '%m-%d') = date_format(r.open_date, '%m-%d')
...
DETERMINISTIC
BEGIN
DECLARE sum_reports INT;

SELECT
	COUNT(r.id) INTO sum_reports
FROM
	`reports` AS r
...
SELECT udf_get_reports_count(1,4);
...
report_id INT)
   ...
   DECLARE category_department_id INT;
	...
	SET employee_department_id := (
		SELECT e.department_id FROM `employees` AS e WHERE e.id = employee_id);
	SET report_category_id :=(
		SELECT r.category_id FROM `reports` AS r WHERE r.id = report_id);
	SET category_department_id := (
		SELECT c.department_id FROM `categories` AS c WHERE c.id = report_category_id);
...
FOR EACH ROW
BEGIN
    IF (ISNULL(OLD.close_date) AND ISNULL(NEW.close_date)) THEN
        SET NEW.status_id = 3;
    END IF;
END $$
DELIMITER ;
