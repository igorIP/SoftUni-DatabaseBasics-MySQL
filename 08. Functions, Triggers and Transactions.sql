...
-- Create stored procedure usp_get_employees_salary_above that 
-- accept a number as parameter and return all employees’ first and last names whose salary is above or equal to the given number. 
-- The result should be sorted by first_name then by last_name alphabetically and id ascending.
...
-- town_name as parameter and return the employees’ first and last name that live in the given town. 
-- The result should be sorted by first_name then by last_name alphabetically and id ascending.
...
-- Write a SQL query to delete all employees from the Production and Production Control departments. 
-- Also delete these departments from the departments table.
...
-- as parameters – sum, yearly interest rate and number of years. 
...
-- It should take he account_id and the interest_rate as parameters. 
...
-- money_amount is valid positive number. Work with precision up to fourth sign after decimal point. 
-- The procedure should produce exact results working with the specified precision.
...
-- Consider cases when one of the account_ids is not valid, the amount of money is negative number, outgoing balance
-- is enough or transferring from/to one and the same account. 
